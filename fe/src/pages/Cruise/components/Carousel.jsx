...
function Carousel({ imagesString, thumbnail }) {
  ...
  useEffect(() => {
    if (imagesString) {
      ...
      const allImages =
        thumbnail && !imagesArray.includes(thumbnail)
  ...
  const visibleThumbnails = useMemo(() => {
    ...
    if (start < 0) {
      ...
      return Array.from({ length: 5 }, (_, i) => images.length - 5 + i).filter(
        ...
        <div className={cx("thumbnails-row")}>
          {visibleThumbnails.map((index) => (
            ...
            >
              ...
              />
              {index === activeIndex && (
                <div className={cx("active-indicator")}></div>
              )}
            </div>
          ))}
        </div>
