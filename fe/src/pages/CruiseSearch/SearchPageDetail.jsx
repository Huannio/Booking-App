...
import { handleGetShipsActiveApi } from "~/api";
import { formatMoney } from "~/utils/formatters";

const cx = classNames.bind(styles);

function SearchPageDetail() {
    const [cruises, setCruises] = useState([]);
    const [sortOption, setSortOption] = useState('none'); // 'none', 'price_asc', 'price_desc'
    ...
    const getCruises = useCallback(async () => {
        try {
            ...
            if (sortOption === 'price_asc') {
                sortParams = { sortBy: 'default_price', sortOrder: 'ASC' };
            } else if (sortOption === 'price_desc') {
                sortParams = { sortBy: 'default_price', sortOrder: 'DESC' };
            }
            
            const response = await handleGetShipsActiveApi(sortParams);
    ...
    return (
        <section id={cx("SearchPageDetail")}>
            <div className={cx("container", "SearchPageDetail-section")}>
                <SectionHeader
                    title={<h4>Tìm thấy {totalResults} kết quả</h4>}
                    mainContent={
                        <div className={cx("sort-container")}>
                            <button 
                                ...
                                onClick={() => {
                                    const newSort = 
                                        sortOption === 'none' ? 'price_asc' :
                                        sortOption === 'price_asc' ? 'price_desc' : 'none';
                                    handleSortChange(newSort);
                                }}
                            >
                                <span>
                                    {sortOption === 'none' && 'Không sắp xếp'}
