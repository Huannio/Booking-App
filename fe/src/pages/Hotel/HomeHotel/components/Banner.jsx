...
function Banner() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    navigate(`/tim-khach-san/search?keyword=${keyword}&location=${location}&price=${price}`);
  };

  return (
    <div className={cx('home-banner')}>
      <video 
        className={cx('home-bg-video')} src="https://res.cloudinary.com/dhnp8ymxv/video/upload/v1732194247/MixivivuHotel_uckfu2.mp4" autoPlay muted loop />
              ...
              <div className={cx('focus-dropdown')}>
                ...
                {locationsDropdownVisible && (
                  <div className={cx('SearchBox-dropdown')}>
                    {['Tất cả địa điểm', 'Bà Rịa - Vũng Tàu', 'Bình Phước', 'Cà Mau', 'Hà Nội', 'Đà Lạt', 'Bình Dương'].map((loc) => (
                      <div
                        key={loc}
